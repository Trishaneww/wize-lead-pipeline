...
const BOOKING_PATTERN =
  /calendly|acuity|squareup|booksy|setmore|book\b|schedule\b/i;
...
export function extractSignals($: cheerio.CheerioAPI): SiteSignals {
  ...
  return {
    ...
    hasOnlineBooking:
      bookingHref ||
      /book (now|online)|schedule (online|now)|book an appointment/.test(
        bodyText,
      ),
    ctaLabels: [...ctaLabels].slice(0, 8),
    mentionsReviews:
      /review|testimonial|rating|\bstars?\b|what our (customers|clients) say/.test(
        bodyText,
      ),
...
export function deriveFindings(i: FindingInputs): AuditFinding[] {
  ...
  if (!i.hasHttps) {
    findings.push({
      ...
      severity: "high",
      description:
        "Site is not served over HTTPS — browsers show 'Not secure', which makes first-time visitors hesitate.",
    });
  }
  if (!i.hasViewportMeta) {
    findings.push({
      ...
      severity: "high",
      description:
        "No mobile viewport tag — the site likely renders poorly on phones, where most visitors arrive.",
    });
  }
  if (!i.signals.hasTapToCall) {
    findings.push({
      ...
      severity: "medium",
      description:
        "No tap-to-call link — mobile visitors can't call in one tap.",
    });
  }
  if (!i.signals.hasContactForm && !i.signals.hasTapToCall) {
    findings.push({
      ...
      severity: "high",
      description:
        "No obvious way to get in touch (no contact form and no tap-to-call).",
    });
  }
  if (!i.signals.mentionsReviews) {
    findings.push({
      key: "no_reviews_surfaced",
      severity: "low",
      description:
        "No reviews or testimonials surfaced — trust isn't visible to a first-time visitor.",
    });
  }
  if (
    i.signals.copyrightYear !== null &&
    i.signals.copyrightYear < new Date().getFullYear() - 2
  ) {
    findings.push({
      key: "stale_copyright",
      severity: "low",
      description: `Footer copyright says ${i.signals.copyrightYear} — the site looks untended.`,
    });
  }
  if (!i.hasTitle) {
    findings.push({
      ...
      severity: "low",
      description:
        "Homepage has no <title> — hurts search visibility and browser tabs.",
    });
  }

  const egregiouslySlow =
    (i.performanceScore !== null &&
      i.performanceScore < EGREGIOUS_PERF_SCORE) ||
    (i.lcpMs !== null && i.lcpMs > EGREGIOUS_LCP_MS);
  if (egregiouslySlow && i.lcpMs !== null) {
    findings.push({
      key: "egregiously_slow",
      severity: "high",
      description: `The site is extremely slow on mobile — main content takes ${(i.lcpMs / 1000).toFixed(1)}s to appear, long enough that many visitors leave first.`,
    });
  }
